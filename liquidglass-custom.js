...
void main() {
	...
	vec3 L1 = normalize(vec3(u_lightDir, 1.0));
	vec3 H1 = normalize(L1 + V);
	float sp1 = pow(max(dot(N, H1), 0.0), u_lightSharp);
	...
	float innerGlow = smoothstep(5.0, 0.0, -sdf) * u_edgeHL * 0.15 * u_innerGlow;
...
var GlassRenderer = class {
  ...
  renderGlassPanel(config, width, height, dpr) {
    ...
    const _ang = config.lightAngle * Math.PI / 180;
